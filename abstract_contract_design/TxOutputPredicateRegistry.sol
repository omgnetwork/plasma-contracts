...
contract TxOutputPredicateRegistry {
     ...
     */
    function registerTxOutputPredicate(uint256 _txoutType, address _contractAddress, uint256 _version) external;

    function getTxoutPredicateByVersion(uint256 _txoutType, uint256 _version) external;

    // use this function to upgrade, need to check the whether the version is registered > 2 weeks
    function upgradeTxoutPredicateTo(uint256 _txoutType, uint256 _version) external;

    // this returns the current version
    function getTxoutPredicate(uint256 _txoutType) public view returns (TxOutputPredicate);
}