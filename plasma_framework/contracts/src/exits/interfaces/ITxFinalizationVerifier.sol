 ...
 * @dev We define two kinds of finalization: protocol finalization and standard finalization 
 ...
 *
 * @dev We've chosen to use an interface in our first deployment cycle since there will be no MVP protocol transactions. This means we can remain open to how we will handle MVP in future. For example, we can check the confirm sig by block root hash, or (block root hash, block number) pair, or
 *      even with EIP712 format. Keeping this upgradeable provides flexibility and allows us to test with our real MVP transactions in future.
...
interface ITxFinalizationVerifier {
    ...
    * @dev MoreVp: Allows in-flight tx, so only checks that the transaction exits.
    */
