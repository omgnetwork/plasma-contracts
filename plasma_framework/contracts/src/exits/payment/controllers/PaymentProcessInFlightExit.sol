...
library PaymentProcessInFlightExit {
        ...
        public
    {
        ...
        // Check if any input is spent already. This is required to prevent operator stealing fund.
        ...
        returns (bool)
    {
        ...
        // Thus there is no need to re-check it here.
        ...
        private
    {
        ...
        // If exiting from output, all inputs are considered spent and can only exit from output further on.
        // If exiting from input, to keep things simple, we require all users to piggyback all input at the same time,
        // instead of re-starting the IFE and re-piggybacking their input.
        uint256 inputNumOfTheToken;
        for (uint16 i = 0; i < MAX_INPUT_NUM; i++) {
            if (exit.inputs[i].token == token) {
                inputNumOfTheToken++;
