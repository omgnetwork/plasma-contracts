...
library PaymentV2ProcessInFlightExit {
    ...
    function run(
        ...
        public
    {
        PaymentV2ExitDataModel.InFlightExit storage exit = exitMap.exits[exitId];
        ...
        if (!exit.isCanonical || isAnyInputFinalizedByOtherExit(self.framework, exit, exitId)) {
            for (uint16 i = 0; i < exit.inputs.length; i++) {
                PaymentV2ExitDataModel.WithdrawData memory withdrawal = exit.inputs[i];
            ...
            for (uint16 i = 0; i < exit.outputs.length; i++) {
                PaymentV2ExitDataModel.WithdrawData memory withdrawal = exit.outputs[i];
    ...
    function isAnyInputFinalizedByOtherExit(
        PlasmaFramework framework,
        PaymentV2ExitDataModel.InFlightExit memory exit,
        uint160 exitId
    )
        private
        view
    ...
    function shouldWithdrawInput(
        ...
        PaymentV2ExitDataModel.InFlightExit memory exit,
        PaymentV2ExitDataModel.WithdrawData memory withdrawal,
        address token,
        uint16 index
    )
        private
    ...
    function returnInputPiggybackBonds(
        Controller memory self,
        PaymentV2ExitDataModel.InFlightExit storage exit,
        address token
    )
        private
    {
    ...
    function allPiggybacksCleared(PaymentV2ExitDataModel.InFlightExit memory exit) private pure returns (bool) {
        for (uint16 i = 0; i < exit.inputs.length; i++) {
