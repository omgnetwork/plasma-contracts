...
contract('LiquidityContract - Fast Exits - End to End Tests', ([_deployer, _maintainer, authority, bob, richFather]) => {
    ...
    const aliceTransferSomeEthToLC = async () => {
        ...
        const outputLC = new PaymentTransactionOutput(OUTPUT_TYPE_PAYMENT, TRANSFER_AMOUNT, this.liquidity.address, ETH);
        const outputAlice = new PaymentTransactionOutput(
            OUTPUT_TYPE_PAYMENT, DEPOSIT_VALUE - TRANSFER_AMOUNT, alice, ETH,
        );
    ...
    describe('Given Alice deposited ETH and transferred some value to the Liquidity Contract', () => {
        ...
        describe('When Bob tries to start the exit from the UTXO created by Alice', () => {
            it('should not allow Bob to start the exit', async () => {
                ...
                const depositUtxoPos = this.depositUtxoPos;
                const rlpDepositTx = this.depositTx;
                const depositInclusionProof = this.merkleProofForDepositTx;

                await expectRevert( this.liquidity.startExitOnRootchainContract(
                    utxoPos, rlpOutputTx, outputTxInclusionProof, rlpDepositTx, depositInclusionProof, depositUtxoPos, { from: bob, value: this.startStandardExitBondSize }
                ),
        ...
        describe('When Alice tries to start the exit with a fake transaction', () => {
            it('should not allow to start exit', async () => {
                ...
                await expectRevert( this.liquidity.startExitOnRootchainContract(
                    utxoPos, rlpOutputTx, outputTxInclusionProof, rlpDepositTx, depositInclusionProof, depositUtxoPos, { from: alice, value: this.startStandardExitBondSize }
                ),
        ...
        describe('When Alice tries to start the standard exit from the Liquidity Contract with all valid proofs', () => {
            before(async () => {
                ...
                await this.liquidity.startExitOnRootchainContract(
                    utxoPos, rlpOutputTx, outputTxInclusionProof, rlpDepositTx, depositInclusionProof, depositUtxoPos, { from: alice, value: this.startStandardExitBondSize }
                );
            });
            it('should start the exit successully', async () => {
                ...
                const outputId = computeNormalOutputId(
                    this.transferTx, outputIndexForTransfer, this.transferUtxoPos,
                );
        ...
        describe('And then Alice processes the exits after two weeks', () => {
            ...
            it('should return the output amount plus standard exit bond to the Liquidity Contract', async () => {
                ...
                const expectedLCBalance = this.LCBalanceBeforeProcessExit
