...
contract(
    'PaymentExitGame - Standard Exit - End to End Tests',
            ...
            describe('Given Alice deposited with ETH', () => {
                ...
                describe('When Alice starts standard exit on the deposit tx', () => {
                    ...
                    it('should put the exit data into the queue of framework', async () => {
                        ...
                        const exitableAt = await this.exitableHelper.calculateDepositTxOutputExitableTimestamp(
                            currentTimestamp,
                        );
                        ...
                        const priorityExpected = await this.exitPriorityHelper.computePriority(
                            exitableAt,
                            utxoPosToTxPos(this.depositUtxoPos),
                            exitIdExpected,
                        );
            ...
            describe('Given Alice deposited ETH and transferred some to Bob', () => {
                ...
                describe('When Alice tries to start the standard exit on the deposit tx', () => {
                    before(async () => {
                        ...
                        this.exitId = await this.exitGame.getStandardExitId(true, this.depositTx, this.depositUtxoPos);
                    });
                    ...
                    describe('Then Bob can challenge the standard exit spent', async () => {
                        before(async () => {
                            ...
                            this.challengeTx = await this.exitGame.challengeStandardExit(args, { from: bob });
                        });

                        it('should challenge it successfully', async () => {
                            await expectEvent.inLogs(this.challengeTx.logs, 'ExitChallenged', {
                        ...
                        it('should transfer the bond and bounty to Bob', async () => {
                            ...
                            expect(actualBobBalanceAfterChallenge).to.be.bignumber.equal(
                                expectedBobBalanceAfterChallenge,
                            );
                        });
            ...
            describe('Given Alice deposited with ERC20 token', () => {
                ...
                describe('Given ERC20 token added to the PlasmaFramework', () => {
                    ...
                    it('should have the ERC20 token', async () => {
                        expect(await this.framework.hasExitQueue(config.registerKeys.vaultId.erc20, this.erc20.address))
                            .to.be.true;
                    });
                    ...
                    describe('When Alice starts standard exit on the ERC20 deposit tx', () => {
                        ...
                        describe('And then Bob processes the exits for the ERC20 token after a week', () => {
                            ...
                            it('should return the standard exit bond in ETH to Alice', async () => {
                                ...
                                const expectedAliceEthBalance = this.aliceEthBalanceBeforeProcessExit.add(
                                    this.startStandardExitBondSize,
                                );

                                expect(actualAliceEthBalanceAfterProcessExit).to.be.bignumber.equal(
                                    expectedAliceEthBalance,
                                );
                            });
                            ...
                            it('should return the process exit bounty in ETH to Bob', async () => {
                                ...
                                expect(actualBobEthBalanceAfterProcessExit).to.be.bignumber.equal(
                                    expectedBobEthBalance,
                                );
                            });

                            it('should return ERC20 token with deposited amount to Alice', async () => {
                                const actualAliceErc20BalanceAfterProcessExit = new BN(
                                    await this.erc20.balanceOf(alice),
                                );
                                const expectedAliceErc20Balance = this.aliceErc20BalanceBeforeProcessExit.add(
                                    new BN(DEPOSIT_VALUE),
                                );

                                expect(actualAliceErc20BalanceAfterProcessExit).to.be.bignumber.equal(
