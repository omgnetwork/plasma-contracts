...
contract('PaymentInFlightExitRouter', ([_, alice, inputOwner, nonInputOwner, outputOwner, nonOutputOwner]) => {
    ...
    describe('piggybackInFlightExit', () => {
        beforeEach(async () => {
        ...
        describe('piggyback on output', () => {
            ...
            it('should fail when not send with the bond value', async () => {
                ...
                await expectRevert(
                    this.exitGame.piggybackInFlightExit(data.outputOneCase.args),
            ...
            it('should fail when no exit to piggyback on', async () => {
                ...
                await expectRevert(
                    this.exitGame.piggybackInFlightExit(
                        data.outputOneCase.args, { from: outputOwner, value: PIGGYBACK_BOND },
                    ),
            ...
            it('should fail when output index exceed max size of tx output', async () => {
                ...
                data.outputOneCase.args.index = MAX_OUTPUT_SIZE + 1;
                await expectRevert(
                    this.exitGame.piggybackInFlightExit(
                        data.outputOneCase.args, { from: outputOwner, value: PIGGYBACK_BOND },
                    ),
                    'Index exceed max size of the input or output',
                );
            });
            ...
            it('should fail when the same output has been piggybacked', async () => {
                ...
                await expectRevert(
                    ...
                    'The indexed input/output has been piggybacked already',
                );
            });
            ...
            it('should fail when the output guard handler of the type is not registered', async () => {
                ...
                await expectRevert(
                    this.exitGame.piggybackInFlightExit(
                        data.outputTwoCase.args, { from: outputOwner, value: PIGGYBACK_BOND },
                    ),
            ...
            it('should fail when not called by the exit target of the output', async () => {
                ...
                await expectRevert(
                    this.exitGame.piggybackInFlightExit(
                        data.outputOneCase.args, { from: nonOutputOwner, value: PIGGYBACK_BOND },
                    ),
            ...
            describe('When piggyback successfully', () => {
                beforeEach(async () => {
                    ...
                    this.piggybackTx = await this.exitGame.piggybackInFlightExit(
                        this.testData.outputOneCase.args, { from: outputOwner, value: PIGGYBACK_BOND },
                    );
                });
                ...
                it('should not enqueue when it is not first piggyback of the exit on the token', async () => {
                    ...
                    await this.exitGame.piggybackInFlightExit(
                        this.testData.outputTwoCase.args, { from: outputOwner, value: PIGGYBACK_BOND },
                    );
                ...
                it('should set the exit as piggybacked on the output index', async () => {
                    ...
                    const positionToFlag = MAX_INPUT_SIZE + this.testData.outputOneCase.args.index;
                ...
                it('should set the correct exit target to withdraw data on the output of exit data', async () => {
                    const exitData = await this.exitGame.getInFlightExitOutput(
                        this.testData.exitId, this.testData.outputOneCase.args.index,
                    );
        ...
        describe('piggyback on input', () => {
            ...
            it('should fail when not send with the bond value', async () => {
                ...
                await expectRevert(
                    this.exitGame.piggybackInFlightExit(argsInputOne),
                    'Input value mismatches with msg.value',
                );
            });
            ...
            it('should fail when input index exceed max size of tx input', async () => {
                ...
                data.argsInputOne.index = inputIndexExceedSize;
                await expectRevert(
                    this.exitGame.piggybackInFlightExit(
                        data.argsInputOne, { from: inputOwner, value: PIGGYBACK_BOND },
                    ),
                ...
                it('should emit InFlightExitPiggybacked event', async () => {
                    await expectEvent.inTransaction(
                        ...
                        'InFlightExitPiggybacked',
                        {
                            ...
                            isPiggybackInput: true,
                            index: new BN(this.testData.argsInputOne.index),
                        },
                    );
                });
            });
