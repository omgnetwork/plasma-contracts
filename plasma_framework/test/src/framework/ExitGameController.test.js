...
contract('ExitGameController', () => {
    ...
    describe('enqueue', () => {
        ...
        it('increases `exitQueueNonce` one', async () => {
        ...
        it('inserts the new unique priority to the queue', async () => {
            ...
            const uniquePriority = await this.dummyExitGame.uniqueuPriorityFromEnqueue();
    ...
    describe('processExits', () => {
        ...
        it('rejects when the "top unique priority" mismatch with the specified one', async () => {
            await this.dummyExitGame.enqueue(0, this.dummyToken, this.dummyExit);
        ...
        it('processes when the "top unique priority" is set to the exact top one of the queue', async () => {
            await this.dummyExitGame.enqueue(0, this.dummyToken, this.dummyExit);
            const uniquePriority = await this.dummyExitGame.uniqueuPriorityFromEnqueue();
        ...
        it('calls the "processExit" function of the exit processor when processes', async () => {
            ...
            // 'exitIdProcessed' would be set when processExit is called in the DummyExitGame
            const exitIdProcessed = await this.dummyExitGame.exitIdProcessed();
            expect(exitIdProcessed).to.be.bignumber.equal(new BN(this.dummyExit.exitId));
        });

        it('deletes the processed exit from queue after processed', async () => {
            await this.dummyExitGame.enqueue(0, this.dummyToken, this.dummyExit);
            ...
            const priorityQueueAddress = await this.controller.exitsQueues(this.dummyToken);
            const priorityQueue = await PriorityQueue.at(priorityQueueAddress);
            expect(await priorityQueue.currentSize()).to.be.bignumber.equal(new BN(0));
        });
